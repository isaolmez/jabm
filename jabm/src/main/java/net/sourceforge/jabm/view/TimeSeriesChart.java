...
/**
 * A graphical report that renders an underlying time series ({@link Timeseries} ) as a JFreeChart time series chart.
 * This report listens for events from the underlying Timeseries object and notifies its chart whenever the series is
 * updated.
 *
 * @author Steve Phelps
 */
public class TimeSeriesChart implements XYDataset, ReportWithGUI, Serializable,
  InitializingBean {
    ...
    /**
     * When this report is computed it notifies its listeners (typically the JFreeChart swing component) that its data
     * set has changed.
     */
